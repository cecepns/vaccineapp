...
const PatientForm = () => {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      if (isEditMode) {
        try {
          setLoading(true);
          const response = await axios.get(`https://api.kingcreativestudio.my.id/api/patients/${id}`, {
  ...
  const onSubmit: SubmitHandler<PatientFormData> = async (data) => {
    try {
      ...
      if (isEditMode) {
        await axios.put(`https://api.kingcreativestudio.my.id/api/patients/${id}`, data, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setSuccess('Patient updated successfully!');
      } else {
        await axios.post('https://api.kingcreativestudio.my.id/api/patients', data, {
          headers: { Authorization: `Bearer ${token}` }
        });
